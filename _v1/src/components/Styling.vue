<template>
  <div
    ...
    :key="renderKey"
    :class="
      selected && selected.motion ? 'timeline-wrapper' : 'keypoint-wrapper'
    "
  >
    ...
    >
      <label
        ...
        v-if="!nest"
        @click="$emit('motion', parent), $emit('parent', null)"
      >
        Back
      </label>
    ...
    <button
      v-if="
        selected && !selected.motion && typeof selected.index !== 'undefined'
      "
      style="width: auto; float: right; margin: 8px 10px 0; padding: 5px"
      class="add-btn"
      @click="removeMotion()"
    >
      Delete
    ...
    <div v-if="store.current" style="color: #eee" class="style-keys">
      <div :key="key" v-for="(value, key) in style">
      ...
      <div v-if="selected && typeof selected.index !== 'undefined'">
        <span
          style="font-size: .8em; padding: 6px 0 2px 5px; background: #888; color: #fff; width: calc(50% - 10px); text-align: left; float: left; margin: 0"
        >
...
export default {
  ...
  computed: {
    style() {
      if (this.selected && this.selected.style) return this.selected.style
      if (this.selected && this.selected.current) return this.selected.current
  ...
  methods: {
    ...
    add() {
      if (this.selected.motion.length) {
        this.selected.motion.push(
          JSON.parse(
            JSON.stringify(
              this.selected.motion[this.selected.motion.length - 1]
            )
          )
        )
    ...
    append(key, value) {
      ...
      if (this.selected && this.selected.style) obj = this.selected.style
      if (this.selected && this.selected.current) obj = this.selected.current
      obj[key] = value
